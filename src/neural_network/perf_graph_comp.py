...
def graph(rand_db, adaboost_db, filename, ttl):
    db = db_interface(rand_db)
    rand_data = db.read()
    db = db_interface(adaboost_db)
    adaboost_data = db.read()
    save_fig(rand_data, adaboost_data, filename, ttl)

def save_fig(rand_data, adaboost_data, filename, ttl):
