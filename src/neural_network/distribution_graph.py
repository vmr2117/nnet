...
def save_fig(data, filename_prefix):
    ...
    for k, g in groupby(n_weights, itemgetter(2)):
        _, _, _, mean, std, perc = map(np.array, zip(*g))
        x = range(len(mean))
        ax.plot(x, mean, label= 'Layer '+str(i+1) + ' mean', color =
                colors[i % len(colors)])
        ax.plot(x, -1 * std, '^', label= 'Layer '+str(i+1) + ' std dev', color
                = colors[i % len(colors)])
        i += 1
    ...
    ax.legend(loc = 'center right', bbox_to_anchor=(1.0, 0.75)) 
    ax.set_xlabel('Epochs')
    ax.set_ylabel('Bias')
    ax.set_title('Biases over training epochs')
    ax.grid()
    plt.savefig(filename_prefix+'_biases.png')
